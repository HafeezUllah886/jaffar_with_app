                    ...
                    <table class="table" id="buttons-datatables">
                        ...
                        <tbody>
                            @foreach ($orders as $key => $order)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td>{{ $order['order_booker']['name'] ?? 'N/A' }}</td>
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end">
                                                <li>
                                                    <button class="dropdown-item"
                                                        onclick="newWindow('{{ route('orders.show', $order['id']) }}')">
                                                ...
                                                @if ($order['status'] == 'Pending')
                                                    @if (auth()->user()->role == 'Admin')
                                                        <li>
                                                            <button class="dropdown-item"
                                                                onclick="newWindow('{{ route('order.sale', $order['id']) }}')">
                                                                <i class="ri-eye-fill align-bottom me-2 text-muted"></i>
                                                                Finalize Order
                                                            </button>
                                                        </li>
                                                    ...
                                                    @if (auth()->user()->role == 'Admin' || $order['orderbookerID'] == Auth::id())
                                                        <li>
                                                            <a class="dropdown-item"
                                                                onclick="newWindowMobile('{{ route('orders.edit', $order['id']) }}')">
